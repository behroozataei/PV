    ...
    internal class RPCCalculation
    {
		...
		public bool Preset1Min()
		{
			...
			try
			{
				...
				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK1"), (float)1))
				{
					result = false;
					_logger.WriteEntry( "Could not update value in SCADA: MARK1", LogLevels.Error);
				}

				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK2"), (float)1))
				{
					result = false;
					_logger.WriteEntry( "Could not update value in SCADA: MARK2", LogLevels.Error);
				}

				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK3"), (float)1))
				{
					result = false;
					_logger.WriteEntry( "Could not update value in SCADA: MARK3", LogLevels.Error);
				}

				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK4"), (float)1))
				{
					result = false;
					_logger.WriteEntry( "Could not update value in SCADA: MARK4", LogLevels.Error);
				}

				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK5"), (float)1))
				{
					result = false;
					_logger.WriteEntry( "Could not update value in SCADA: MARK5", LogLevels.Error);
				}

				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK6"), (float)1))
				{
					result = false;
					_logger.WriteEntry( "Could not update value in SCADA: MARK6", LogLevels.Error);
				}

				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK7"), (float)1))
				{
					result = false;
					_logger.WriteEntry( "Could not update value in SCADA: MARK7", LogLevels.Error);
				}

				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK8"), (float)1))
				{
					result = false;
				...
				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK9_1"), (float)1))
				{
					result = false;
					_logger.WriteEntry( "Could not update value in SCADA: MARK9_1", LogLevels.Error);
				}

				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK9_2"), (float)1))
				{
					result = false;
					_logger.WriteEntry( "Could not update value in SCADA: MARK9_2", LogLevels.Error);
				}

				if (!_updateScadaPointOnServer.WriteAnalog(_repository.GetRPCScadaPoint("MARK10"), (float)1))
				{
					result = false;
